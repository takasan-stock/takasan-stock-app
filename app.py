...
def _fetch_fundamentals_one(ticker: str) -> dict:
    """
    1銘柄分の売上5y CAGRと売上成長予想を取得する。
    yfinanceの一時的な失敗に備えて2回までリトライする。
    """
...
def enrich_with_fundamentals(df: pd.DataFrame, max_workers: int = 4) -> pd.DataFrame:
    """
    合致銘柄に対して、売上5y CAGRと売上予想を並列取得して追加する。
    取得に失敗した銘柄数をst.warningで報告する。
    """
    if df.empty:
        df["売上5y CAGR"] = []
        df["売上予想"] = []
        ...
        for fut in as_completed(futures):
            ...
            if r["err"]:
                ...
                if len(fail_samples) < 3:
                    fail_samples.append(f"{r['ticker']}: {r['err']}")
    ...
    if fail_count > 0:
        st.warning(
            f"⚠️ ファンダメンタルズ取得: {fail_count}/{len(tickers)} 銘柄で失敗しました。"
            f"（例: {' | '.join(fail_samples)}）"
            "Yahoo Finance側のレート制限・データ未提供が主な原因です。"
        )
...
with st.sidebar:
    ...
    if scan_mode.startswith("お試し"):
        ...
        SCAN_LIMIT = None  # 全銘柄
        st.caption(
            "⏱ 目安: 約15〜20分（500銘柄=2.6分の実測値から換算）。\n\n"
            "⚠️ Streamlit Cloud無料枠は長時間実行でセッション切断や"
            "メモリ不足が起きやすく、途中で結果が欠ける可能性があります。"
            "全銘柄を安定して回したい場合は、GitHub Actions等で"
            "バックグラウンド実行し、結果だけをこのアプリで表示する構成を推奨します。"
        )
